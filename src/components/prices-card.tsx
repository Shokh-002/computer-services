    ...
    <section className="py-10 px-4">
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        {services.map((service, index) => (
          <Card key={index} className="bg-[#00205B] text-white p-0 rounded-3xl">
            <CardContent className=" flex flex-col p-0 pt-4 pl-4 justify-between w-full h-full rounded-2xl ">
             ...
             <Link href="#" className=" w-20 text-lime-400  text-sm font-medium flex items-center gap-1">
                Batafsil →
              </Link>
              <Image
                src={service.image}
                ...
                width={235}
                height={184}
                className=" ml-11.5  object-contain"
              />
             </div>
            </CardContent>
